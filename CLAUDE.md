...
**Ella MDM1** ist ein leichtgewichtiges Master Data Management System für lokale Edge-Geräte,
angebunden an die Cloud-Plattform Ella-Energy (AWS).

**Zweck:** Verwaltung von Projekten, Devices, Alarmstufen, E-Mail-Adressen und Lookup-Werten.
...
│   │   ├── dashboard/           # Dashboard mit 6 Menü-Kacheln
...
Jede Tabelle enthält: `create_user`, `create_timestamp`, `modify_user`, `modify_timestamp`, `modify_status`
...
- `auditInsert(user)` → setzt alle 5 Felder
...
Alle Hauptbereiche (Projekte, Devices, Alarme) folgen dem Muster:

1. **Suchmaske** — Suchfelder + Button "Suche"
2. **Ergebnisliste** — Tabelle, Klick auf Zeile → Detail
3. **Detailseite/Dialog** — Formular mit Tabs für komplexe Entitäten
...
- **Freigabe** (grüner Button, erscheint statt Sperren) → `PUT { action: "unlock" }` → `modify_status = 'updated'`
- Alle Buttons (Speichern, Löschen, Kopieren) sind auch im gesperrten Zustand sichtbar und nutzbar
...
| Alarmstufen | Liste + Hinzufügen/Löschen von Alarmstufen |
| Ziel-E-Mails | Liste + Hinzufügen/Löschen von E-Mail-Adressen |
...
| Alarm | Alarm-Meldung (0/1), Alarm-Stufe, Alarm-Zeitpunkt |
| Technische Daten | JSON-Freifeld |

---

## Aktueller Implementierungsstand (Version 1 — Stand 2026-04-09)

| Bereich | Status | Anmerkung |
|---|---|---|
| Login / Signup | ✅ fertig | better-auth, Email+Password |
| Dashboard | ✅ fertig | 6 Menü-Kacheln |
| Projekte | ✅ fertig | Suche, Liste, Detail (5 Tabs), Kopieren, Sperren/Freigabe |
| Devices | ✅ fertig | Suche, Liste, Detail (5 Tabs), Sperren/Freigabe |
| Alarme | ✅ fertig | Suche, Liste, Detail-Dialog (Neu/Bearbeiten/Löschen) |
| Import | ✅ fertig | CSV-Upload |
...
3. **Device kopieren** — analog zu Projekt-Kopieren mit editierbarem Key
4. **Alarme in Projekt-Detail** — bereits als Tab vorhanden, nutzt `GET /api/alarms?project_name=`
5. **Cloud-Sync** — REST-API-Anbindung an Ella-Energy (AWS)
6. **Passwort ändern / Benutzer-Verwaltung** — aktuell nur Signup
